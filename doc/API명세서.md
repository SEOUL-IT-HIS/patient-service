...
| `safetyInfoId` | path | UUID | 상세·수정·비활성화·고정 및 해제 시 필수. 해당 환자 소유의 안전정보 식별자 |
...
시각은 고정 소수점 자릿수를 보장하지 않는다. 변경 직후 응답과 DB 재조회 응답의 소수초 정밀도가 다를 수 있으므로 문자열 완전 일치로 변경 여부를 판단하지 않는다. 이미 비활성인 정보의 반복 비활성화는 DB에 저장된 수정시각을 갱신하지 않는다.
...
`safetyNote`는 필수 문자열이다. 서버가 UUID와 생성·수정시각을 생성하고 `activeYn=Y`, `pinnedYn=N`으로 저장한다.
...
| `400` | 경로 UUID·쿼리 includeInactive 변환 실패 | `요청값이 올바르지 않습니다. 파라미터: ..., 입력값: ...` |
| `400` | 본문 누락·잘못된 JSON | `요청 데이터 형식이 올바르지 않습니다.` |
| `404` | 환자 미존재 | `환자 정보를 찾을 수 없습니다.` |
| `404` | 안전정보 미존재 또는 다른 환자 소유 | `환자 안전정보를 찾을 수 없습니다.` |
| `400` | pinned 누락·null | Bean Validation의 NotNull 기본 메시지(실행 환경 언어에 따라 다름) |
| `409` | 활성 안전정보 2건 고정 상태에서 추가 고정 | `안전정보는 최대 2건까지 고정할 수 있습니다. 기존 고정을 해제해 주세요.` |
| `409` | 비활성 안전정보 수정·고정·해제 | `비활성화된 환자 안전정보는 수정할 수 없습니다.` |
...
활성 정보만 고정·해제할 수 있고 환자당 최대 2건이다. 같은 상태의 반복 요청은 성공한다.
